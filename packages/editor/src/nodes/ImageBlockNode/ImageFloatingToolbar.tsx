...
import { $isImageBlockNode, ImageBlockNode } from './ImageBlockNode';
import {
  AlignCenter,
  AlignLeft,
  AlignRight,
...
export function ImageFloatingToolbar({
  ...
  layout,
  alignment,
}: ImageFloatingToolbarProps): React.JSX.Element {
  const updateNode = useCallback(
    (updateFn: (node: ImageBlockNode) => void) => {
      editor.update(() => {
  ...
  const handleAlignment = (newAlignment: ImageBlockAlignment) => {
