...
const COLOR_OPTIONS: { value: StatusColor; label: string; variant: StatusPillVariant }[] = [
                        ...
                        <div className="flex gap-2">
                            {COLOR_OPTIONS.map((option) => (
                                <button
                                    ...
                                    className={`status-color-swatch ${color === option.value ? 'status-color-swatch-selected' : ''
                                        }`}
                                    title={option.label}
                                    aria-label={`Set status color to ${option.label}`}
                                    aria-pressed={color === option.value}
                                >
                                    <StatusPill variant={option.variant} className="text-xs px-2 py-0.5">
                                        {option.label[0]}
                                    </StatusPill>
                                </button>
                            ))}
