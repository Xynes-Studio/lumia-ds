...
describe('fileUtils', () => {
    ...
    describe('isFileTypeAllowed', () => {
        ...
        test('returns true for exact match', () => {
            expect(isFileTypeAllowed('image/png', ['image/png', 'image/jpeg'])).toBe(true);
        });

        test('returns false for non-match', () => {
            expect(isFileTypeAllowed('video/mp4', ['image/png', 'image/jpeg'])).toBe(false);
        });
    ...
    describe('getFileIcon', () => {
        ...
        test('returns document for word files', () => {
            ...
            expect(getFileIcon('application/vnd.openxmlformats-officedocument.wordprocessingml.document')).toBe('document');
        });
